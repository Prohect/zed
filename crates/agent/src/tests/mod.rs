...
async fn test_non_blocking_tool_call(cx: &mut TestAppContext) {
    ...
    let placeholder: serde_json::Value =
        serde_json::from_str(&tool_result_text("call_slow")).unwrap_or_else(|_| {
            panic!("not json: {:?}", tool_result_text("call_slow"))
        });
    assert_eq!(placeholder["status"], json!("running"));
    let async_id = placeholder["async_tool_call_id"].as_str().unwrap().to_string();
    assert!(
    ...
    let MessageContent::Text(text) = &last_message.content[0] else {
        panic!("expected a text user message, got {:?}", last_message.content);
    };
